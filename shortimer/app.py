...
from shortimer.cache import concept2, hybrid
...
from shortimer.cache.gym import refresh_all_configured
from shortimer.cache.parse import prune_expired_parses
...
from shortimer.router import (
    concept2 as concept2_router,
)
from shortimer.router import (
    hybrid as hybrid_router,
)
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    background = [
        ...
        asyncio.create_task(
            _refresh_loop(
                "concept2",
                concept2.REFRESH_INTERVAL_SECONDS,
                concept2.refresh_concept2_cache,
                concept2.ensure_wods_parsed,
            )
        ),
        ...
        asyncio.create_task(
            _refresh_loop(
                "hybrid",
                hybrid.REFRESH_INTERVAL_SECONDS,
                hybrid.refresh_hybrid_cache,
                hybrid.ensure_wods_parsed,
            )
        ),
...
app = FastAPI(title="shortimer", version="0.1.0", lifespan=lifespan)
...
app.include_router(concept2_router.router)
